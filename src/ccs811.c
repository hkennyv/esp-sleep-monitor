...
{
    /*
     * Register 0x20 "hardware id"
     * 
     * contains the chip identificatio number chip_id[7:0], which is 0x81
    ...
    i2c_master_write_byte(cmd, (CCS811_ADDR << 1) | I2C_MASTER_WRITE, I2C_ACK_CHECK_EN); // WRITE
    i2c_master_write_byte(cmd, reg_addr, I2C_ACK_CHECK_EN);                              // reg_addr

    i2c_master_start(cmd);                                                              // START
    i2c_master_write_byte(cmd, (CCS811_ADDR << 1) | I2C_MASTER_READ, I2C_ACK_CHECK_EN); // READ
